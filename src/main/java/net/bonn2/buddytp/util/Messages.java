...
public class Messages {
    ...
    private static final File messagesFile = new File(BuddyTP.plugin.getDataFolder() + File.separator + "messages.txt");
    ...
    public static void load() {
        ...
        try {
            String[] lines = new String(BuddyTP.plugin.getClass().getResourceAsStream("/messages.txt").readAllBytes()).split("\n");
        ...
        if (!messagesFile.exists()) {
            try (FileWriter writer = new FileWriter(messagesFile, false)) {
                writer.write(new String(BuddyTP.plugin.getClass().getResourceAsStream("/messages.txt").readAllBytes()));
            } catch (IOException e) {
                throw new RuntimeException(e);
            }
        }
        ...
        for (String key : defaultMessages.keySet()) {
            if (!messages.containsKey(key)) {
                missingKeys = true;
                BuddyTP.plugin.getLogger().warning("Missing translation key %s default value will be written to messages.txt");
